...
public class Optimal {
    ...
    private List<int[]> getPairs(List<int[]> a, List<int[]> b, int target) {
        ...
        TreeMap<Integer,List<int[]>> tragetPairs = new TreeMap<>();

        for (int i = 0; i < a.size(); i++) {
            for (int j = 0; j < b.size(); j++) {
                int val = a.get(i)[1] + b.get(j)[1];
                if (val <= target) {
                    if (tragetPairs.containsKey(val)) {
                        tragetPairs.get(val).add(new int[]{a.get(i)[0], a.get(i)[0]});
                    } else {
                        var pair = new ArrayList<int[]>();
                        pair.add(new int[]{a.get(i)[0], a.get(i)[0]});
                        tragetPairs.put(val, pair);
                    }
                }
            }
        }

        return tragetPairs.floorEntry(target).getValue();
...
/**
 ...
 and as close to target as possible. Return a list of ids of selected elements. If no pair is possible, return an empty list.
